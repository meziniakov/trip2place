...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  const parks: RootObject = await fetcher(
    `https://opendata.mkrf.ru/v2/parks/$?f={"nativeId":{"$eq":${params?.id?.toString()}}}`
  )

  return {
    props: {
      id: params?.id,
      data: parks.data[0]?.data.general,
    },
  }
}

export const getStaticPaths: GetStaticPaths = async () => {
  // const parks: RootObject = await getAllMuseums('1')
  const parks: RootObject = await fetcher(
    'https://opendata.mkrf.ru/v2/parks/$'
  )

  const paths = parks.data.map((museum) => ({
